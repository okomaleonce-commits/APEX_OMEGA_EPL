...
def get_tier(team_name):
    if team_name in {"Arsenal", "Manchester City", "Liverpool"}: return 1
    if team_name in {"Chelsea", "Newcastle United", "Manchester United", "Tottenham"}: return 2
    if team_name in {"Aston Villa", "Brighton", "Fulham", "West Ham", "Bournemouth"}: return 3
    if team_name in {"Brentford", "Crystal Palace", "Everton", "Wolves"}: return 4
    return 5
...
def _acl_check(injured):
    ...
    return {"active": flag is not None, "factor": factor, "flag": flag, "by_line": by_line}


def _lineup_dict(injured):
    players = [{"name": p.get("name", ""), "line": "defense"} for p in injured]
    return {"injured_players": players, "acl_check": _acl_check(players), "lineup_confirmed": False, "ais_f_raw": 0.0}


def build_match_ctx(fix, home_inj, away_inj, h2h, home_stats, away_stats, home_tier, away_tier):
    h2h_avg = sum(m.get("total_goals", 0) for m in h2h) / len(h2h) if h2h else 0.0
    return {
        "home_name": fix["home_team"], "away_name": fix["away_team"],
        ...
        "home_lineup": _lineup_dict(home_inj), "away_lineup": _lineup_dict(away_inj),
        "h2h_matches": h2h, "h2h_avg_goals": round(h2h_avg, 2),
        "home_te_goals": 0, "home_te_hours": 999, "away_te_goals": 0, "away_te_hours": 999,
        "away_eba_active": False, "away_eba_victory": False, "away_eba_rotations": 0,
        "home_raw_odds": None, "away_raw_odds": None,
...
async def run_pipeline():
    ...
    try:
        from ingestion.fixtures_service import fetch_upcoming, fetch_team_stats, fetch_injuries, fetch_h2h
        from ingestion.odds_service import get_odds_for_match, get_reference_odds
        from ingestion.enrichment_service import enrich_match
        ...
        total_signals = 0
        for fix in fixtures:
            home_name, away_name = fix["home_team"], fix["away_team"]
            kickoff, fixture_id = fix["kickoff_utc"], fix["fixture_id"]
            ...
            try:
                ...
                if not odds_1x2:
                    odds_1x2, odds_ou25 = get_reference_odds(home_name, away_name, home_stats, away_stats)

                n_inj_home, n_inj_away = len(home_inj), len(away_inj)
                ais_home = max(-0.25, -0.05 * min(n_inj_home, 5))
                ais_away = max(-0.25, -0.05 * min(n_inj_away, 5))
                home_tier, away_tier = get_tier(home_name), get_tier(away_name)

                dcs = 55
                if home_stats.get("matches_played", 0) > 0: dcs += 10
                if away_stats.get("matches_played", 0) > 0: dcs += 10
                dcs += 15 if odds_1x2.get("source") == "api_football" else 5
                if h2h: dcs += 5
                if n_inj_home > 0 or n_inj_away > 0: dcs += 5

                xg_home, xg_away = compute_xg(home_stats, away_stats, home_capacity=fix.get("venue_capacity", 0), ais_f_home=ais_home, ais_f_away=ais_away)
                model = run_simulation(xg_home, xg_away)

                enriched = enrich_match(home_name, away_name, kickoff, model, odds_1x2, odds_ou25)
                model = enriched["model"]
                odds_1x2 = enriched["odds_1x2"]
                odds_ou25 = enriched["odds_ou25"]
                odds_btts = enriched["odds_btts"]
                odds_source = enriched["odds_source"]
                enrichment_source = enriched["enrichment_source"]
                dcs = min(100, dcs + enriched.get("dcs_bonus", 0))

                probs = {"home": model["home"], "draw": model["draw"], "away": model["away"]}
                match_ctx = build_match_ctx(fix, home_inj, away_inj, h2h, home_stats, away_stats, home_tier, away_tier)
                match_ctx["home_raw_odds"] = odds_1x2.get("home_raw", 1.5)
                match_ctx["away_raw_odds"] = odds_1x2.get("away_raw", 2.5)
                adj_probs, moratoriums, rules_active, xg_home, xg_away, multi_rule = apply_all_rules(match_ctx, probs, xg_home, xg_away)
                model["home"], model["draw"], model["away"] = adj_probs.get("home", model["home"]), adj_probs.get("draw", model["draw"]), adj_probs.get("away", model["away"])

                verdicts = generate_verdicts(model, odds_1x2, odds_ou25, dcs, moratoriums=moratoriums, multi_rule_active=multi_rule, odds_btts=odds_btts)
                ...
                if verdicts:
                    for v in verdicts:
                        save_signal({
                            ...
                            "market": v.get("market", ""), "outcome": v.get("label", v.get("market", "")),
                            "model_prob": v.get("model_prob", 0), "demargin_prob": v.get("demargin_prob", 0),
                            "raw_odds": v.get("raw_odds", 0), "edge": v.get("edge", 0),
                            "max_stake_pct": v.get("max_stake_pct", 0), "status": v.get("status", ""),
                            "dcs_score": dcs, "xg_home": xg_home, "xg_away": xg_away,
                            "odds_source": odds_source,
                        })
                    total_signals += len(verdicts)
                else:
                    save_no_bet(fixture_id, home_name, away_name, kickoff, dcs, xg_home, xg_away)

                msg = format_verdict_telegram(home_name, away_name, kickoff, model, verdicts, dcs, n_inj_home=n_inj_home, n_inj_away=n_inj_away, odds_source=odds_source, rules_active=rules_active, moratoriums=moratoriums, enrichment_source=enrichment_source)
...
async def main():
    ...
    await send_channel("APEX-OMEGA-EPL v1.4 operationnel\n" f"Date: {datetime.now(timezone.utc).strftime('%d/%m/%Y %H:%M')} UTC\n" "Sources: API-Football + FootyStats optionnel + Odds-API.io optionnel\n" "Commandes: /start /help /status /analyse /bilan /api /refresh")
    await run_pipeline()
...
if __name__ == "__main__":
    asyncio.run(main())
# v1.4 — FootyStats + Odds-API.io enrichment
