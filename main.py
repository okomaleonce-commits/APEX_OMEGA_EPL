...
async def send_channel(text):
    """Envoie un message sur le canal principal."""
...
def get_tier(team_name):
    """Retourne le Tier EPL d'une equipe (1-5)."""
...
def build_match_ctx(fix, home_inj, away_inj, h2h,
    ...
    """Construit le contexte match pour apply_all_rules."""
...
async def run_pipeline():
    ...
    for fix in fixtures:
        ...
        try:
            ...
            multi_rule = len([r for r in rules_active
                              if any(t in r for t in
                                     ["R6","R7","R8","R9","R10","R11"])]) >= 2
            ...
            logger.error(f"Erreur analyse {home_name} vs {away_name}: {e}")
            import traceback
...
async def main():
    ...
    # Application Telegram avec polling
    ...
    logger.info("Polling actif — commandes disponibles")

    # Message de demarrage sur le canal
    await send_channel(
        ...
        "Pipeline auto: 08h/14h/20h UTC"
    )

    # Analyse immediate au demarrage
    await run_pipeline()
